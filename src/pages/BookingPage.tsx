        ...
        return (
          <div key={s.key} className="flex items-center gap-1 flex-1">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className={`text-xs font-medium truncate ${isActive ? 'text-foreground' : 'text-muted-foreground'}`}>{s.label}</span>
            </div>
...
function CallDetails({ selectedDate, selectedTime }: { selectedDate?: Date; selectedTime: string | null }) {
  return (
    <div className="space-y-5">
      <div>
        <span className="text-[10px] uppercase tracking-widest text-primary font-bold">AI Capital Raising</span>
        <h3 className="text-lg font-bold text-foreground mt-1 leading-tight">Discovery Call</h3>
        <p className="text-sm text-muted-foreground mt-1">Learn how we help funds raise $1M–$50M+ from accredited investors using AI-driven systems.</p>
      </div>
...
export default function BookingPage() {
  ...
  const { calendarId, slots, loadingSlots } = useCalendarSlots({ route: '/book', selectedDate });
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await supabase.functions.invoke('lead-intake', {
        body: { name, email, phone, source: 'booking-page', status: 'new' },
      });
    } catch (err) {
    ...
    <div className="min-h-screen bg-muted/30 text-foreground">
      <header className="bg-background border-b border-border">
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground"><Shield className="w-3.5 h-3.5" /><span>256-bit encrypted</span></div>
        </div>
      </header>
      ...
      <main className="px-4 py-8 md:py-12">
        <div className="max-w-4xl mx-auto">
          <div className="bg-background rounded-2xl border border-border shadow-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-border bg-muted/30"><BookingStepIndicator step={step} /></div>
              ...
              <div className="flex flex-col lg:flex-row">
                ...
                <div className="flex-1 p-6 lg:p-8">
                  <form onSubmit={handlePhoneSubmit} className="space-y-5">
                    <div><h2 className="text-xl font-bold text-foreground">Enter your details</h2><p className="text-sm text-muted-foreground mt-1">We'll capture your info first, then you'll pick a time.</p></div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1.5"><label className="text-sm font-medium text-foreground">Full Name</label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="John Smith" required className="h-11" /></div>
                      <div className="space-y-1.5"><label className="text-sm font-medium text-foreground">Email</label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="john@example.com" required className="h-11" /></div>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-foreground">Phone Number <span className="text-destructive">*</span></label>
                      <div className="relative"><Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1 (555) 000-0000" required className="h-11 pl-10" /></div>
                    </div>
                    <Button type="submit" size="lg" className="w-full h-12 font-semibold text-base gap-2" disabled={!isPhoneValid}>Continue to Schedule <ArrowRight className="w-4 h-4" /></Button>
                  </form>
                  <div className="mt-6 relative rounded-xl overflow-hidden border border-border">
                    <div className="absolute inset-0 bg-background/60 backdrop-blur-sm z-10 flex items-center justify-center">
                      <div className="bg-background/90 border border-border rounded-lg px-5 py-3 shadow-sm flex items-center gap-2"><CalendarIcon className="w-4 h-4 text-primary" /><span className="text-sm font-medium text-foreground">Fill in your details to unlock calendar</span></div>
                    </div>
                    <div className="p-4 opacity-40 pointer-events-none select-none">
                      <div className="flex gap-6">
                        <Calendar mode="single" disabled={() => true} fromDate={today} toDate={addDays(today, 60)} className="rounded-lg" />
                        <div className="space-y-2 min-w-[120px] hidden sm:block">
                          {previewSlots.map((time) => (<div key={time} className="px-4 py-2.5 rounded-lg border border-border text-sm text-muted-foreground">{time}</div>))}
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col lg:flex-row">
                <div className="lg:w-72 shrink-0 p-6 lg:p-8 lg:border-r border-border">
                  ...
                  <div className="mt-6 pt-5 border-t border-border">
                    <div className="flex items-center gap-2 text-xs text-muted-foreground"><div className="w-2 h-2 rounded-full bg-primary animate-pulse" /><span>Booking for <span className="text-foreground font-medium">{name}</span></span></div>
                    <p className="text-xs text-muted-foreground mt-1 truncate">{email}</p>
                    <button onClick={() => setStep('phone')} className="text-xs text-primary hover:underline mt-2 font-medium">Edit details</button>
                  </div>
                </div>
                <div className="flex-1 p-6 lg:p-8">
                  ...
                  <div className="flex flex-col lg:flex-row gap-6">
                    <Calendar mode="single" selected={selectedDate} onSelect={(d) => { setSelectedDate(d); setSelectedTime(null); }} disabled={disabledDays} fromDate={today} toDate={addDays(today, 60)} className="rounded-xl border border-border" />
                    {selectedDate && (
                      <div className="space-y-2 min-w-[150px]">
                        ...
                        {loadingSlots ? (
                          ...
                          slots.map((time) => (
                            <button key={time} onClick={() => setSelectedTime(time)} className={`w-full px-4 py-3 rounded-lg border text-sm font-medium transition-all ${selectedTime === time ? 'bg-primary text-primary-foreground border-primary shadow-md' : 'border-border text-foreground hover:border-primary/50 hover:bg-primary/5'}`}>{time}</button>
                          ))
                        ) : (
                  ...
                  {selectedDate && selectedTime && (
                    <div className="mt-8">
                      <Button size="lg" className="w-full sm:w-auto h-12 font-semibold text-base gap-2 px-10" onClick={handleConfirm} disabled={loading}>
                        {loading ? (<><Loader2 className="w-4 h-4 animate-spin" /> Booking...</>) : (<>Confirm Booking <ArrowRight className="w-4 h-4" /></>)}
                      </Button>
                    </div>
                  )}
                </div>
